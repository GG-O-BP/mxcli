...
func EnsureDatabase(db DBConfig, w io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("invalid --db-host %q: %w", db.Host, err)
	}
	// An empty host (e.g. --db-host ":5432") means loopback; make it explicit so
	// every downstream psql/pg_isready gets a real -h.
	if host == "" {
		host = "127.0.0.1"
		db.Host = net.JoinHostPort(host, port)
	}
...
// private socket), or the original `sudo -u postgres psql` for a system cluster.
...
func (s superuser) psql(args ...string) *exec.Cmd {
	...
	// reach itself over the socket. -w never prompts, so probing an unrelated
	// password-protected system cluster fails fast instead of hanging.
	host := s.host
	if s.sock != "" {
		host = s.sock
	}
	base := []string{"-v", "ON_ERROR_STOP=1", "-w",
		"-h", host, "-p", s.port, "-U", "postgres", "-d", "postgres"}
